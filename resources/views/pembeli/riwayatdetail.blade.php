...
<div class="max-w-5xl mx-auto mt-8 bg-white p-6 rounded shadow">
    ...
    @if($item->barang)
        @php
            $barang = $item->barang;
            $sudahRating = \App\Models\Rating::where('pembeli_id', auth()->id())
        ...
        <!-- Kotak barang -->
        <div class="border bg-gray-50 relative rounded-none p-3 w-full sm:w-56">
            <img src="{{ asset('images/barang/' . $barang->id . '/' . $barang->thumbnail) }}"
                alt="{{ $barang->nama }}"
                class="w-full h-auto aspect-[4/3] object-cover mb-2 rounded"
                onerror="this.src='{{ asset('images/not-found.jpg') }}'">

            <p class="text-sm font-semibold truncate">{{ $barang->nama }}</p>
            <p class="text-xs text-gray-600">Harga: Rp {{ number_format($barang->harga, 0, ',', '.') }}</p>
            <p class="text-xs">Subtotal: Rp {{ number_format($item->subtotal, 0, ',', '.') }}</p>
        </div>

        <!-- Form rating (di luar kotak barang) -->
        @if($transaksi->status === 'selesai')
            <div class="mt-3 ml-1">
                @if($sudahRating)
                    <p class="text-base text-green-600">⭐ Sudah Dinilai ({{ $sudahRating->rating }} Bintang)</p>
                @else
